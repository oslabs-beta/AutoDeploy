...
router.post('/connect', requireSession, async (req, res) => {
  ...
  console.debug(
    `[AWS CONNECT] Starting SSO credential retrieval for user ${userId} with start URL: ${sso_start_url} and region: ${sso_region}`
  );
  ...
  try {
    ...
    if (!creds || !creds.accessKeyId || !creds.secretAccessKey) {
      console.error(
        '[AWS CONNECT ERROR] Retrieved credentials are incomplete or invalid',
        creds
      );
      return res
        .status(500)
        .json({ error: 'Failed to retrieve valid AWS credentials from SSO' });
    }
    ...
    try {
      ...
      console.debug(
        `[AWS CONNECT] Successfully listed ${Buckets.length} buckets`
      );
    } catch (listErr) {
      ...
      return res
        ...
        .json({
          error:
            'AWS credentials are invalid or lack permissions to list S3 buckets',
        });
    }
    ...
    console.error(
      '[AWS CONNECT ERROR] Unexpected error during AWS connect flow',
      err
    );
    return res
      .status(500)
      .json({
        error:
          'An unexpected error occurred while connecting to AWS: ' +
          err.message,
      });
  }
});
...
router.get('/start', requireSession, async (req, res) => {
  ...
  try {
    ...
    if (!rows.length) {
      ...
      return res
        .status(404)
        .json({ error: 'No AWS SSO start URL found for user' });
    }
    const ssoStartUrl = rows[0].sso_start_url;
    if (
      !ssoStartUrl ||
      typeof ssoStartUrl !== 'string' ||
      ssoStartUrl.trim() === ''
    ) {
      console.warn(`[AWS START] Invalid SSO start URL for user ${userId}`);
      return res
        .status(400)
        .json({ error: 'Invalid AWS SSO start URL for user' });
    }
    ...
    console.debug(
      `[AWS START] Redirecting user ${userId} to AWS SSO start URL: ${redirectUrl}`
    );
    return res.redirect(redirectUrl);
  } catch (err) {
    console.error(
      `[AWS START ERROR] Failed to redirect user ${userId} to AWS SSO start URL`,
      err
    );
    return res
      .status(500)
      .json({
        error: 'Failed to redirect to AWS SSO start URL: ' + err.message,
      });
  }
});
...
router.get('/callback', requireSession, async (req, res) => {
  ...
  try {
    ...
    if (!rows.length) {
      console.warn(
        `[AWS CALLBACK] No AWS connection info found for user ${userId}`
      );
      return res
        .status(404)
        .json({ error: 'No AWS connection info found for user' });
    }
    const { sso_start_url, sso_region, account_id, role_to_assume } = rows[0];
    if (!sso_start_url || !sso_region) {
      console.warn(
        `[AWS CALLBACK] Missing SSO start URL or region for user ${userId}`
      );
      return res
        .status(400)
        .json({ error: 'Missing AWS SSO start URL or region for user' });
    }

    console.debug(
      `[AWS CALLBACK] Attempting to retrieve SSO credentials for user ${userId}`
    );
    ...
    if (!creds || !creds.accessKeyId || !creds.secretAccessKey) {
      console.error(
        `[AWS CALLBACK ERROR] Retrieved credentials are incomplete or invalid for user ${userId}`,
        creds
      );
      return res
        .status(500)
    ...
    console.debug(
      `[AWS CALLBACK] Retrieved SSO credentials successfully for user ${userId}`
    );
    ...
    try {
      ...
      Buckets = response.Buckets || [];
      console.debug(
        `[AWS CALLBACK] Successfully listed ${Buckets.length} buckets for user ${userId}`
      );
    } catch (listErr) {
      console.error(
        `[AWS CALLBACK ERROR] Failed to list S3 buckets for user ${userId}`,
        listErr
      );
      return res
        .status(500)
    ...
    console.debug(
      `[AWS CALLBACK] Stored AWS connection info for user ${userId}`
    );
    ...
    console.error(
      `[AWS CALLBACK ERROR] Unexpected error during AWS callback flow for user ${userId}`,
      err
    );
    return res
      .status(500)
      .json({
        error:
          'An unexpected error occurred while processing AWS callback: ' +
          err.message,
      });
  }
});
...
router.get('/me', requireSession, async (req, res) => {
  ...
  if (!rows.length)
    return res.status(404).json({ error: 'No AWS connection found' });
  res.json(rows[0]);
});
...
router.get('/start-device', requireSession, async (req, res) => {
  ...
  try {
    ...
    console.error(
      '[AWS DEVICE FLOW ERROR] Failed to start device authorization',
      err
    );
    return res
      .status(500)
      .json({ error: 'Failed to start device authorization: ' + err.message });
  }
});
...
router.get('/device-callback', requireSession, async (req, res) => {
  ...
  try {
    ...
    if (!rows.length) {
      return res
        .status(404)
        .send('No pending device authorization session found.');
    }
    const session = rows[0];
    const { client_id, client_secret, device_code, poll_interval } = session;
    const oidc = new SSOOIDCClient({ region });
    let tokenResp = null;
    let attempts = 0;
      ...
      try {
        ...
        if (
          ...
          await new Promise((resolve) =>
            setTimeout(resolve, (poll_interval || 5) * 1000)
          );
          attempts++;
          continue;
        } else {
          console.error(
            '[AWS DEVICE FLOW ERROR] Unexpected error while polling for token',
            err
          );
          return res
            .status(500)
            .send('Failed to obtain AWS device token: ' + err.message);
        }
      }
    }
    if (!tokenResp) {
      return res
        .status(408)
        .send('Device authorization timed out. Please try again.');
    }
    ...
    return res
      .status(500)
      .send('Failed to complete device authorization: ' + err.message);
  }
});
